...
def do_work(connection, channel, delivery_tag, body):
    ...
    subprocess.run(['bash', './gen_encode.sh', path_to_video, path_to_storage])
    print("[x] Done")
    
    payload = {
            "video_name": video_name,
            "data": {
                "streaming_link":"http://" + STREAM_ADDR + '/' + video_name.split('.')[0] + "/playlist.m3u8"  
            },
            "operation_name":"hls"
    }
    ...
    if res.status_code == 201:
        print("HLS_ENC: Written to video name to DB.")
    else:
        print("HLS_ENC: Something went wrong writing to DB: ", res.status_code)
...
channel.queue_declare(queue="hls", durable=True, auto_delete=True)
...
channel.basic_consume(queue='hls', on_message_callback=callback)
