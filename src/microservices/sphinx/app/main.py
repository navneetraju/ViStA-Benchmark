...
def do_work(connection, channel, delivery_tag, body):
    ...
    text=get_large_audio_transcription(VIDEO_DIRECTORY, video_name)
    audio = {video_name.split('.')[0] : text}
    print("[x] Done")
    
    payload = {
            "video_name": video_name,
            "data": audio,
            "operation_name":"audio_analytics"
    }
    ...
    if res.status_code == 201:
        print("audio_analytics: Written to audio text to DB.")
    else:
        print("audio_analytics: Something went wrong writing to DB: ", res.status_code)
...
channel.queue_declare(queue="audio_analytics", durable=True, auto_delete=True)
...
channel.basic_consume(queue='audio_analytics', on_message_callback=callback)
